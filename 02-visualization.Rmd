...
# color aesthetic
ggplot(data = bechdel) +
    geom_point(mapping = aes(x = intgross , y =domgross , color = test))
...
```{r echo=FALSE}
ggplot(data = bechdel) +
  geom_histogram(mapping = aes(x = domgross ))
```
...
ggplot(bechdel, aes(x = binary ,
                  y = budget)) +
  geom_col(aes(fill = binary), width = 0.2)
```
...
bechdel %>% count(year)%>%ggplot() +
 ...
 geom_line(mapping = aes(x = year, y = n),color="pink")
```
...
ggplot(data = bechdel,aes(x = binary))+
    ...
    labs(title = "Graph title", 
         x = "X-axis", 
         y = "Y-axis",
         caption = "data source: Prakriti don")
```
...
```{r}
bechdel %>% count(test) %>% ggplot() +
    geom_col(aes(x=reorder(test,-n),y=n,fill=test))+
    geom_label(aes(x= test, y = n, label = n)) +
    labs(title = "Distribution of clean test", y = "Count",x = "")+
  coord_flip()
```
